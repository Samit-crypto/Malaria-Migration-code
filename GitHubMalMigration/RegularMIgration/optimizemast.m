...
beta1s=0.7*30; %p(1) = biting rate at origin
beta1ds = 0.85*30; %p(2) = biting rate at destination
betaMds= 0.65*30; %p(3) = biting rate for migrants
scalingRs = 0.5; %p(4) scaling factor for Rain
multiRs = 0.55;  %p(5) multiplier factor for Rain 
multiTs = 0.6;  %p(6) multiplier factor for Temperature 
scalingTs = 0.013;  %p(7) scaling factor for temperature
scalingMigs = 0.7;%0.9;  %p(8) scaling factor for migration
p1s = 0.85; %p(9) = reporting probability

 lb = [0.949,0.499,0.0009,0.799,0.199,0.949,0.499,0.0009,0.799,0.199,...
     0.0009,0.0009,0.0009,0.59*30,0.6*30,0.54*30,0.45,0.2,0.599,...
    0.0129,0.5,0.809];
 ub = [0.951,0.501,0.002,0.801,0.201,0.951,0.501,0.002,0.801,0.201,0.002,...
     0.002,0.002,1*30,1*30,1*30,0.6,1,0.65,0.0131,...
    3,1];
...
opts = optimset('Display','iter','Algorithm','SQP','TolX',1e-10000,'TolCon',1e-10000,'TolFun',1e-10000,...
    'MaxIter',0 , 'MaxFunEvals', 1e+1000000); % Sets fminsearch options
